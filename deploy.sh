...
# Убедимся, что рабочая область чиста (закомментировано, т.к. Flutter сам меняет файлы)
...
# Сборка веб-приложения Flutter с указанием базового пути
...
# Сохраняем локальные изменения, которые создал Flutter при сборке
git stash
...
echo "Восстанавливаю сохраненные изменения..."
